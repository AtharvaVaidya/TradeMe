...
public extension RequestProtocol
{
    ...
    func url(in service: ServiceProtocol) throws -> URL
    {
        ...
        var baseURL = service.configuration.url.absoluteString.appending(self.endpoint)//.appending(page == nil ? "" : "&page=\(self.page!)")
        ...
        //Requests that need authentication are the ones that need pagination hence offset is only added if authentication is required
        if authenticationRequired
        {
//            baseURL.append("\(url.query != nil ? "&" : "?")ts=\(authenticationToken.timestamp)&apikey=\(authenticationToken.publicKey)&hash=\(authenticationToken.hash)")
//            
            if let page = page
            {
                baseURL.append("&offset=\(page * 20)")
            }
            
            guard let urlWithAuthentication = URL(string: baseURL)
            else { return url }
            
//            print(urlWithAuthentication)
...
//        if authenticationRequired
//        {
//            let creds = (key: Constants.consumerKey, secret: Constants.consumerSecret)
//            urlRequest.oAuthSign(method: self.method?.rawValue ?? "GET", urlFormParameters: [:], consumerCredentials: creds)
//            let authHeader = OhhAuth.calculateSignature(url: requestURL, method: self.method?.rawValue ?? "GET", parameter: [:], consumerCredentials: creds, userCredentials: nil)
